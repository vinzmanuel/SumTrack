...
import { getPasswordRecoveryPendingChallenge } from "@/lib/auth/password-recovery";
...
export const metadata: Metadata = {
  title: "Verify Password Reset | SumTrack",
};

export default async function ForgotPasswordVerifyPage({
  searchParams,
}: ForgotPasswordVerifyPageProps) {
  const [pendingChallenge, params] = await Promise.all([
  ...
  const channelLabel = pendingChallenge.channel === "email" ? "Email" : "SMS";
          ...
          <div className="space-y-5">
            ...
            {!resolvedError && codeWasSent ? (
              <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-300">
                If an account matches that information, a code has been sent.
              </div>
            ) : null}

            <form action="/forgot-password/verify/submit" className="space-y-5" method="post">
            ...
            <form action="/forgot-password/verify/resend" method="post">
              <button
