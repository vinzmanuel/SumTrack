...
function buildDataUrl(filters: CollectorAssignedLoansFilters) {
  ...
  return query ? `/dashboard/assigned-loans/data?${query}` : "/dashboard/assigned-loans/data";
}
...
function buildAssignedLoansReturnTo(filters: CollectorAssignedLoansFilters) {
  const params = new URLSearchParams();
  ...
  const query = params.toString();
  return query ? `/dashboard/assigned-loans?${query}` : "/dashboard/assigned-loans";
}

export function CollectorSelfAssignedLoansPage({
  initialData,
  initialFilters,
}: {
  ...
  const loadResults = useCallback(async (nextFilters: CollectorAssignedLoansFilters) => {
    ...
    try {
      ...
      setErrorMessage("Unable to refresh your assigned loans right now.");
    } finally {
  ...
  const currentReturnTo = buildAssignedLoansReturnTo(filters);
